...
### **Summary: A Strategic Path to SQL Mastery for Data Science**
---
...
| **DDL** | Data Definition Language | To define and modify the *structure* of database objects. | CREATE, ALTER, DROP, TRUNCATE |
| **DML** | Data Manipulation Language | To manipulate the *data* stored within database objects. | INSERT, UPDATE, DELETE |
...
**Youtube Resource for DDL:** [Data with Baraa](https://www.youtube.com/watch?v=3Qbq61A_sNs)
...
**Youtube Resource for DML:** [Data with Baraa](https://www.youtube.com/watch?v=ku3vMAP0h0s)
...
* **String**: CHAR(n)/Varchar is fixed-length, padding with spaces up to n (e.g., for state abbreviations). VARCHAR(n) is variable-length, using only the space needed *up to* n (ideal for names, comments).
...
* **PRIMARY KEY (PK)**: A constraint that combines NOT NULL and UNIQUE. It serves as the unique identifier for a row in a table. A table can have only one Primary Key. This enforces *entity integrity*.
* **FOREIGN KEY (FK)**: A column (or set of columns) in one table that refers to the PRIMARY KEY in another table. This creates the link between tables and enforces *referential integrity*, ensuring that a row in a "child" table cannot be created without a corresponding row in the "parent" table.27 This PK-FK relationship is the fundamental mechanism that enables relational database joins.
...
A common source of confusion for SQL learners is *why* certain commands work in some places and not others. The reason is the *logical execution order*, which is different from the *written order* of a query. SQL do not execute in top-down line-by-line fashion.
...
* **Comparison Operators**: \=, \!= (Not Equal), \<\> (Not Equal), \>, \<, \>=, \<=  
...
* **BETWEEN**: Filters for values within an *inclusive* range (e.g., WHERE hire\_date BETWEEN '2023-01-01' AND '2023-12-31'). 
* **LIKE**: Used for pattern matching in strings. It uses two wildcards:  
  ...
  * \_: Matches exactly one character (e.g., 'Patr\_ck' matches 'Patrick').
...
* It can sort by one or more columns: ORDER BY department, last\_name.
...
* **CONCAT**: Combines (concatenates) two or more strings into a single string. eg. CONCAT('abc','-','def') ==> abc-def
...
**More on String Functions:** [Geeks for Geeks](https://www.geeksforgeeks.org/sql/sql-string-functions/)
...
**More on Date/Time functions:**[Geeks for Geeks](https://www.geeksforgeeks.org/sql/sql-date-functions/)
...
* **COALESCE(expr1, expr2,...)**: Returns the *first non-null expression* in its argument list. This is a powerful tool for data cleaning and providing default values (e.g., COALESCE(column\_value, 0\) replaces all NULLs with 0).
...
* **Correlated Subquery**: A subquery that references columns from the *outer* query. This type of query is evaluated once *for every row* processed by the outer query, which can be very slow and inefficient.
...
**More on CTE:** [Data with Baraa](https://www.youtube.com/watch?v=5x1uodxEIaM)
...
* **Stored Procedures**: A precompiled collection of one or more SQL statements stored in the database. They can accept parameters, contain complex control-of-flow logic (IF, WHILE), and perform transactions. They are executed as a single call (e.g., EXECUTE sp\_my\_procedure) and *cannot* be used inside a SELECT statement.
* **User-Defined Functions (UDFs)**: Routines that accept parameters and return a value (either a single scalar value or a table). Unlike procedures, UDFs *can* be used inline within SQL statements, just like built-in functions.
...
* **ROW\_NUMBER()**: Assigns a unique, sequential number to each row within its partition (e.g., 1, 2, 3, 4).
...
* **DENSE\_RANK()**: Assigns a rank based on the ORDER BY clause. It gives *tied values the same rank* but *does not skip* subsequent ranks (e.g., 1, 2, 2, 3).
...
SELECT date, sales, LAG(sales, 1\) OVER (ORDER BY date) AS previous\_day\_sales FROM....
...
**More on Window Function:** [Data with Baraa](https://www.youtube.com/watch?v=Wvg4PjbMTO8)
...
1. **Anchor Member**: The base query that runs *once* to establish the starting set of the recursion. This is the "base case" (e.g., SELECT employee\_id, name, 1 AS level FROM employees WHERE manager\_id IS NULL). 
2. **Recursive Member**: The query that runs *iteratively*. It references the CTE itself and joins it to the next level of the hierarchy (e.g., SELECT e.employee\_id, e.name, r.level \+ 1 FROM employees e JOIN recursive\_cte r ON e.manager\_id \= r.employee\_id).
...
**More on Recursive CTE:**[Data with Baraa](https://www.youtube.com/watch?v=5x1uodxEIaM)
...
| :---- | :---- | :---- |
| **B-Tree** | General purpose, OLTP. High-cardinality columns. Range queries (\>, \<, BETWEEN).34 | Low-cardinality columns in DWH (less efficient than Bitmap). |
| **Hash** | Exact equality lookups (=) only. Blazing fast for this one task.34 | Range queries (useless). Frequent updates. |
| **Bitmap** | OLAP / Data Warehouse. Low-cardinality columns (e.g., 'Gender', 'Region'). | OLTP / Write-heavy systems. High-cardinality columns (e.g., 'user\_id'). |
...
### **Module 10: Applied SQL for Data Science and AI**
...
#### **10.1 Pattern 1: Funnel Analysis**

* **Use Case**: Tracking user conversion through a multi-step process. (e.g., view\_homepage $\\rightarrow$ view\_product $\\rightarrow$ add\_to\_cart $\\rightarrow$ purchase).
* **SQL Pattern**: Use a series of sequential CTEs or LEFT JOINs.  
  ...
  4. The final SELECT uses COUNT(DISTINCT step1.user\_id), COUNT(DISTINCT step2.user\_id), etc., to build the funnel counts.
...
* **SQL Pattern**:  
  ...
  5. GROUP BY cohort\_month, month\_number and COUNT(DISTINCT user\_id) to get a retention matrix. provides a specific example of finding a *reactivation* cohort.
...
A 5-Step Framework for any case study:
